...
export default function App() {
    ...
    const initAuth = async () => {
      try {
        ...
        if (error.code === 'auth/configuration-not-found') {
          setToastMessage(
            '⚠️ Firebaseコンソールで「匿名認証」を有効にしてください'
          );
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const unsubFixed = onSnapshot(
      fixedCollection,
      (snapshot) => {
        const fixedData = snapshot.docs.map((doc) => ({
          id: doc.id,
          ...doc.data(),
        }));
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
          ...
          <div className="flex justify-between text-sm mb-3">
            <div className="flex items-center gap-1">
              <div
                className={`w-3 h-3 rounded-full ${USERS.user1.color}`}
              ></div>
              <span className="text-gray-600">
                {USERS.user1.name}: ¥{stats.u1Total.toLocaleString()}
              </span>
            </div>
            <div className="flex items-center gap-1">
              <span className="text-gray-600">
                ¥{stats.u2Total.toLocaleString()} :{USERS.user2.name}
              </span>
              <div
                className={`w-3 h-3 rounded-full ${USERS.user2.color}`}
              ></div>
            </div>
          </div>
          ...
          <div className="bg-gray-50 rounded-xl p-3 flex justify-between items-center text-xs text-gray-500 font-medium border border-gray-100">
            ...
            <span className="bg-white px-2 py-1 rounded-md shadow-sm border border-gray-200 text-[10px]">
              {settings.splitMethod === 'ratio'
                ? `ルール: ${settings.user1Ratio}:${100 - settings.user1Ratio}`
                : '金額固定'}
            </span>
              ...
              {stats.total === 0 ? (
                ...
                <p className="font-bold text-gray-800 text-sm">
                  ...
                  <span className="text-teal-600 text-lg">
                    ¥{Math.abs(stats.u1Diff).toLocaleString()}
                  </span>{' '}
                  もらう
                </p>
              ) : stats.u1Diff < 0 ? (
            ...
            <div className="space-y-3">
              {stats.categoryTotals.map((c) => {
                ...
                return (
                  <div
                    key={c.id}
                    className="flex items-center gap-3 bg-gray-50 p-2.5 rounded-2xl"
                  >
                    ...
                    <div className="flex-1">
                      <div className="flex justify-between text-sm">
                        ...
                        <div className="text-right">
                          ...
                          <span className="text-xs text-gray-400 font-medium ml-2 w-8 inline-block">
                            {percentage}%
                          </span>
                        </div>
                      </div>
  ...
  const AddTransactionView = () => {
    // 新規追加時は「今日」の日付にするが、ホームで過去月を見ていればその月に合わせるという配慮も可能。
    // 今回はシンプルに現在の日付をデフォルトにする。
                ...
                return (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-3 rounded-2xl transition-all border-2 ${
                      isSelected
                        ? 'border-teal-500 bg-teal-50 scale-105 shadow-sm'
                        : 'border-transparent bg-white shadow-sm hover:bg-gray-50'
                    }`}
                  >
                    <div
                      className={`p-2 rounded-full mb-1 ${
                        isSelected ? 'bg-teal-500 text-white' : c.color
                      }`}
                    >
                      <Icon size={20} />
                    </div>
                ...
                <div key={month} className="space-y-3">
                  ...
                  {groupedTx[month].map((t) => {
                    ...
                    return (
                      <div
                        key={t.id}
                        className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex items-center gap-3 transition-all hover:shadow-md"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            <span
                              className={`text-[10px] px-2 py-0.5 rounded-full ${user.lightColor}`}
                            >
                              {user.name}
                            </span>
                          </div>
  ...
  const FixedExpensesView = () => {
    ...
    const handleDeleteFixed = async (id) => {
      ...
      try {
        await deleteDoc(
          doc(db, 'artifacts', appId, 'public', 'data', 'fixedExpenses', id)
        );
      ...
      <div className="p-5 h-full overflow-y-auto pb-32 animate-in fade-in duration-300">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <span
                      className={`inline-block mt-1 text-[10px] px-2 py-0.5 rounded-full ${user.lightColor}`}
                    >
                      {user.name}が支払う
                    </span>
                  </div>
        ...
        {!isAdding ? (
          ...
          <div className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 mt-4 animate-in fade-in slide-in-from-top-2">
            <h4 className="font-bold text-gray-700 mb-4 text-sm">
              新しい固定費を登録
            </h4>
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:outline-none"
                  placeholder="何代？"
                />
              </div>
              <div className="flex gap-3">
                <div className="flex-1">
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border border-gray-200 rounded-xl bg-white focus:ring-2 focus:ring-teal-500 focus:outline-none text-sm font-medium"
                  >
                    {Object.values(USERS).map((u) => (
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button
                  ...
                  disabled={isSaving}
                  className="flex-1 py-3 bg-teal-600 text-white font-bold rounded-xl disabled:opacity-50 hover:bg-teal-700 transition-colors"
                >
                  追加する
                </button>
              </div>
          ...
          {method === 'ratio' ? (
                ...
                <div className="flex items-center gap-4 mb-6">
                  <div className="flex-1 text-center">
                    ...
                    <input
                      ...
                      value={ratio}
                      onChange={(e) =>
                        setRatio(Math.min(100, Math.max(0, e.target.value)))
                      }
            ...
            <div className="space-y-6 animate-in fade-in">
              <div>
                ...
                >
                  <option value="user1">{USERS.user1.name}</option>
                  <option value="user2">{USERS.user2.name}</option>
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-600 mb-2">
                ...
                <div className="relative">
                  <span className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-500 font-bold text-xl">
                    ¥
                  </span>
                  <input
                    type="number"
      ...
      <nav className="bg-white border-t border-gray-100 pb-safe absolute bottom-0 w-full z-20">
        <div className="flex justify-around items-center h-20 px-1 pb-2">
          ...
          >
            <div
              className={`p-1.5 rounded-full transition-all duration-300 ${
                activeTab === 'home' ? 'bg-teal-50 scale-110' : ''
              }`}
            >
          ...
          >
            <div
              className={`p-1.5 rounded-full transition-all duration-300 ${
                activeTab === 'history' ? 'bg-teal-50 scale-110' : ''
              }`}
            >
          ...
          >
            <div
              className={`p-1.5 rounded-full transition-all duration-300 ${
                activeTab === 'fixed' ? 'bg-teal-50 scale-110' : ''
              }`}
            >
              <CalendarCheck
                size={22}
                strokeWidth={activeTab === 'fixed' ? 2.5 : 2}
              />
            </div>
          ...
          >
            <div
              className={`p-1.5 rounded-full transition-all duration-300 ${
                activeTab === 'settings' ? 'bg-teal-50 scale-110' : ''
              }`}
            >
              <Settings
                size={22}
